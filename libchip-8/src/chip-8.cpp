...
std::string_view chip_8_version()
{
    return LIBCHIP8_VERSION;
}
...
void Chip8::execute(Opcode opcode)
{
    ...
    case opcode_id::SKIEQ:
        skieq(opcode.X, opcode.NN);
        break;
    case opcode_id::SKINE:
        skine(opcode.X, opcode.NN);
        break;
    case opcode_id::SKREQ:
        skreq(opcode.X, opcode.Y);
        break;
    case opcode_id::MOVI:
        movi(opcode.X, opcode.NN);
        break;
    case opcode_id::ADDI:
        addi(opcode.X, opcode.NN);
        break;
    case opcode_id::MOV:
        mov(opcode.X, opcode.Y);
        break;
    case opcode_id::OR:
        _or(opcode.X, opcode.Y);
        break;
    case opcode_id::AND:
        _and(opcode.X, opcode.Y);
        break;
    case opcode_id::XOR:
        _xor(opcode.X, opcode.Y);
        break;
    case opcode_id::ADD:
        add(opcode.X, opcode.Y);
        break;
    case opcode_id::SUB:
        sub(opcode.X, opcode.Y);
        break;
    case opcode_id::SRL:
        srl(opcode.X, opcode.Y);
        break;
    case opcode_id::LSUB:
        lsub(opcode.X, opcode.Y);
        break;
    case opcode_id::SLL:
        sll(opcode.X, opcode.Y);
        break;
    case opcode_id::SKRNE:
        skrne(opcode.X, opcode.Y);
        break;
    case opcode_id::MOVRI:
        movri(opcode.NNN);
        break;
    case opcode_id::JMPV0:
        jmpv0(opcode.NNN, opcode.X);
        break;
    case opcode_id::RAND:
        rand(opcode.X, opcode.NN);
        break;
    case opcode_id::DRAW:
        draw(opcode.X, opcode.Y, opcode.N);
        break;
    case opcode_id::SKKP:
        skkp(opcode.X);
        break;
